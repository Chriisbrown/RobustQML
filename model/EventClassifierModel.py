...
class ECModel(ABC):
    """Parent Class for Event Classifier Models
...
class ECModelFactory:
    """The factory class for creating Anomlay Detection Models"""

    registry = {}
    """ Internal registry for available Event Classifier Models """

    @classmethod
    def register(cls, name: str):
        """Decorator for registering new Event Classifier models

        Args:
            name (str): Name of the model
        """

        def inner_wrapper(wrapped_class: ECModel):
            if name in cls.registry:
                print('Event Classifer Model %s already exists. Will replace it', name)
    ...
    @classmethod
    def create_ECModel(cls, name: str, folder: str, config: dict, **kwargs) -> 'ECModel':
        """Factory command to create the Event Classifier Model"""
        ec_class = cls.registry[name]
        model = ec_class(folder, **kwargs)
